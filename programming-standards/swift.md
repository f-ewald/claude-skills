...
### `Logger` `.debug` messages missing from the host's `log stream`/Console.app on Simulator
...
Simulator; likely applies to iOS/tvOS Simulator too), the host's usual
instructions don't fully work, because two different things are conflated:
...
1. `sudo log config --mode level:debug --subsystem <id>` on the **host** only
   ...
   `xcrun simctl spawn booted log config --subsystem <id> --mode level:debug`
   (no `sudo` needed — the directory is user-owned; `--mode
   level:debug,persist:debug` makes no difference here). Verify with
   `xcrun simctl spawn booted log config --subsystem <id> --status`.
2. Even with that simulator-local policy confirmed as `DEBUG`, `.debug`-level
   messages still never reached the **host's** `log stream`/Console.app in
   testing — only `.info` and above did, and this persisted across a full
   `simctl shutdown`/`boot`. The host's `log stream`/Console.app relays
   Simulator logs through a path that silently caps at `info`, independent of
   the Simulator's own policy.

**The fix**: don't stream from the host at all — stream **from inside the
Simulator's own context**, which reads directly from its log daemon and
bypasses the host relay entirely:

```
xcrun simctl spawn booted log stream --level debug --predicate 'subsystem == "<id>"'
```

This showed every `.debug` line correctly once the simulator-local policy
above was set. (`simctl spawn <udid>` also works in place of `booted`.)

**How to apply:** if debug logs are missing from Console.app/`log stream` on
Simulator, don't assume it's the app's logging code first. To rule the app
out, write a minimal host-side Swift executable (`swiftc` a few lines using
the same `Logger(subsystem:category:)` and levels) and check it against a
plain host `log stream --level debug` — if that shows `.debug` fine, the gap
is Simulator-specific relay behavior. Then: (1) set the Simulator's own
subsystem policy with `simctl spawn booted log config --subsystem <id> --mode
level:debug`, and (2) stream with `simctl spawn booted log stream --level
debug --predicate '...'` instead of a bare host-side `log stream`. Both steps
matter — either alone isn't enough.
